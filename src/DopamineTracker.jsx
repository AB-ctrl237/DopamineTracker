...
const BADGES = [
  ...
  },
  {
    ...
    desc: "10 earned activities in a day",
    check: (s) => s.maxEarnedInDay >= 10,
  },
  {
    id: "focus_master",
    label: "Focus Master",
  ...
  {
    id: "discipline_beast",
    label: "Discipline Beast",
    desc: "30-day streak",
    check: (s) => s.bestStreak >= 30,
  },
  {
    id: "dopamine_warrior",
    label: "Dopamine Warrior",
    desc: "100 earned activities",
...
function computeStats(activities) {
  ...
  sortedDays.forEach((d) => {
    if (byDay[d].earned > byDay[d].instant) {
      streak++;
  ...
  if (
    ...
    while (
      i >= 0 &&
      byDay[sortedDays[i]].earned > byDay[sortedDays[i]].instant
    ) {
      currentStreak++;
      i--;
    }
  ...
  const maxEarnedInDay = Math.max(
    0,
    ...Object.values(byDay).map((d) => d.earned),
  );
...
function scoreMessage(score) {
  if (score < 0)
    return { msg: "You're chasing quick rewards.", color: COLORS.instant };
  ...
  return (
    <div style={S.card}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: "1rem" }}>
        {["instant", "earned"].map((t) => (
          <button
            ...
            style={{
              ...S.btn(t === "instant" ? COLORS.instant : COLORS.earned),
              flex: 1,
              opacity: type === t ? 1 : 0.35,
              fontSize: 13,
            }}
          >
      ...
      <button
        ...
        disabled={
          !category || (category.startsWith("custom_") && !customLabel.trim())
        }
        style={{
          ...S.btn(type === "instant" ? COLORS.instant : COLORS.earned),
          width: "100%",
          opacity:
            !category || (category.startsWith("custom_") && !customLabel.trim())
              ? 0.4
  ...
  return (
    <div style={S.card}>
      ...
      >
        ...
        <input
          type="date"
          style={{ ...S.input, flex: 1, minWidth: 130, colorScheme: "dark" }}
          value={filterDate}
          onChange={(e) => setFilterDate(e.target.value)}
        />
        {filterDate && (
          <button style={S.btnOutline} onClick={() => setFilterDate("")}>
            Clear
          </button>
        )}
      </div>
            ...
            >
              <div
                style={{
                  ...
                  borderRadius: "50%",
                  background:
                    a.type === "instant" ? COLORS.instant : COLORS.earned,
                  flexShrink: 0,
                }}
              />
    ...
    >
      ...
      <div style={S.card}>
        ...
        {pieData.length === 0 ? (
          ...
          >
            Log activities to see the split.
          </div>
        ) : (
          <ResponsiveContainer width="100%" height={200}>
            <PieChart>
              ...
              >
                {pieData.map((_, i) => (
                  <Cell
                    key={i}
                    fill={i === 0 ? COLORS.instant : COLORS.earned}
                  />
                ))}
              </Pie>
              <Tooltip contentStyle={tipStyle} />
...
export default function DopamineTracker() {
  ...
  useEffect(() => {
    save("dt_activities", activities);
  }, [activities]);

  const addActivity = useCallback((act) => {
    setActivities((prev) => [
      ...prev,
      { ...act, id: Date.now() + Math.random() },
    ]);
    setMsgKey((k) => k + 1);
  }, []);
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = "dopamine_tracker.csv";
    a.click();
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <img
              src="src\assets\favicon-black-512.png "
              className="logo"
              alt=""
              srcset=""
            />
          ...
          <div style={{ display: "flex", gap: 4 }}>
            {NAV_TABS.map((t) => (
              <button
                ...
                style={{
                  background:
                    activeTab === t.id ? COLORS.accent + "22" : "transparent",
                  border:
                    activeTab === t.id
        ...
        {activeTab === "dashboard" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={S.card}>
              ...
              >
                <span style={{ color: COLORS.instant }}>
                  {" "}
                  Instant ({stats.todayInstant})
                </span>
                <span style={{ color: scoreColor, fontWeight: 700 }}>
                  {scoreMsg}
                </span>
                <span style={{ color: COLORS.earned }}>
                  {" "}
                  Earned ({stats.todayEarned})
                </span>
              </div>
              <div
                style={{
              ...
              >
                {stats.todayInstant + stats.todayEarned === 0
                  ? "Log activities to see your balance."
                  : `${Math.round((stats.todayEarned / (stats.todayInstant + stats.todayEarned)) * 100)}% earned today`}
              </div>
            </div>
